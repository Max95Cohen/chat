...
const FIRE_BASE_URI = 'https://fcm.googleapis.com/fcm/send';
...
$allNotify = $allNotify = $redis->zRange("all:notify:queue", 0, -1);
...
while (true) {
    foreach ($allNotify as $notify) {
        Co\run(function () use ($notify,$redis,$fireBaseApiKey){
            sendPushForAllChatMembers($notify,$redis,$fireBaseApiKey);
        });
...
function sendPushForAllChatMembers( string $notify, Redis $redis, string $fireBaseApiKey) {
    ...
    if ($notifyData) {
        ...
        if ($message) {
            ...
            $messageText = $attachments ? MessageHelper::getAttachmentTypeString($message['type']) : $text;
            ...
            if ($chat && is_array($chatMembers)) {
                ...
                foreach ($chatMembers as $k => $chatMember) {
                    ...
                    curl_setopt($ch, CURLOPT_URL, FIRE_BASE_URI);
